...
version: 1
slug: "src-bridge-c"
primary_target: "src/bridge.c"
related_targets: ["src/platform.zig","src/application.zig"]
---
...
STORY: The reader sees the document first, scans a compact page index without
losing place, jumps directly when needed, enters annotation mode, writes, and
sees local save confirmation.

FIRST VIEWPORT: A dark folio bar sits above an almost full-height PDF; a narrow
scrolling page index sits at left, while a contextual light margin folds from
the right edge and carries the Done action.
