...
| `ui/layout.zig` | Toolbar, panel, rail, page rectangle, hover, scrollbar thumb and its inverse mapping | Points outside every region | No overlapping buttons at any width, panel rows that never overlap from the minimum window height upward, quarter edges, scroll clamps, no scrollbar when pages fit |
| `ui/input.zig` | Keys, clicks, wheel, drags, files, scrollbar grab, drag, release, and track clicks | Unknown keys, empty paths | Strokes closed by hovers, hidden rail, page rectangle edges, wake and leave events, drags that win over the pen |
...
| `tests/end_to_end_tests.zig` | A whole session through native events: choose a file, draw with the pen, drag the rail scrollbar, turn pages, bookmark, switch theme, persist, reopen; the production loop until quit | Quit with nothing open | Worker renders and thumbnails landing, files in the formats the next session reads |
